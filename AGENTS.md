...
Phase 6 JD Structured AI Analysis and Phase 8 Interview Record & Feedback Loop are complete and accepted. Phase 7 Resume Version & Evidence Map remains `IMPLEMENTED — SEMANTIC ACCEPTANCE PAUSED`. Phase 9 Profile Vault & Safe Job Form Autofill is owner-approved on `phase/9-safe-autofill`. Deliver one local singleton Profile, user-triggered current-form scan, deterministic resolution, preview and confirmed fill while preserving `Scan != Fill != Submit`. Stop before the next phase. Do not change Phase 6/7 prompts, schemas, datasets or evaluation; do not add file parsing/upload, resume generation or whole-resume rewriting, matching/ATS/Offer scores, recommendations, RAG, embeddings, vector databases, Agent frameworks, AI form recognition or answers, automatic submission/continue/agreements, another recruitment platform, cloud sync, authentication, telemetry or analytics frameworks.

## Canonical context

Read ADR-008 through ADR-016, `tasks/plan.md`, `tasks/todo.md`, README, PRODUCT_SPEC, ARCHITECTURE, API_CONTRACT, DATA_MODEL, ROADMAP, ENGINEERING_PRINCIPLES and the files in `docs/technical/`. Historical remote-identity experiments exist only at `pre-local-first-cleanup` and are not current design input.
...
- Preserve SQLite/SQLAlchemy/Alembic and existing runtime data; never delete, replace, inspect through tests, or commit `runtime-data/jobpilot.db`. Phase 9 may add only one singleton `autofill_profiles` table for the approved minimal structured facts; it remains separate from Resume Version and Application.
...
- Extension bundles all code, never modifies proxies, and uses only `activeTab`, `scripting` and the exact loopback host. Job capture and form scan/fill require distinct user gestures against the current rendered DOM; there is no background/content script, recruitment-site host permission, broad URL permission or persistent Profile storage.
