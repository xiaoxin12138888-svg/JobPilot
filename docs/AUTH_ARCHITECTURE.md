...
Only a hash of the identifier is stored server-side. The session record contains the local `user_id`, provider identity, creation/last-use/absolute-expiry times, revocation state, and only the minimum provider grant material needed for renewal or upstream revocation. Provider refresh material, if retained, is encrypted at rest with a server-managed key and is never logged.
...
`intent=reauth` additionally binds the transaction to the current Web session/User and sends `prompt=login` plus `max_age=0`. Callback requires the same `(issuer, subject)` and a validated recent `auth_time`, then rotates/upgrades that session; it cannot switch accounts. Ordinary `intent=login` also uses `prompt=login`, so a local JobPilot logout cannot be silently undone by a surviving Auth0 SSO cookie.
...
- **Revoke all sessions** requires recent authentication. It revokes every local Web session and all provider refresh grants for the identity. The UI must state the maximum residual access-token window rather than promise instantaneous JWT invalidation.
