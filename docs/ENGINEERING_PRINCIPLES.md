...
3. **Loopback only**：API、客户端和 PostgreSQL 工具只接受 loopback；不提供公网/LAN fallback。
4. **One fact source**：未来持久业务状态由本机 API/domain 与 PostgreSQL 定义。
...
- 数据库 URL 必须是 loopback PostgreSQL；
- 操作系统账户与文件权限保护本地静态数据；
