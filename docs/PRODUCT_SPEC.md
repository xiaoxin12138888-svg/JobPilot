...
> 只有 14/20 Provider 请求获得有效响应，合并成功率未达 19/20。2026-09-16 完成的 V2
> BOSS/牛客人工内容验收为 6/6 PASS，但不能替代冻结 synthetic 门槛，因此不得标记 PASS 或进入
> Phase 12。
...
只有用户明确点击“编辑简历”后才按相同分区逐行显示原文编辑框。保存时只替换用户编辑的分区内容，
未编辑的标题、顺序、换行和其他正文保持原样；无明确标题的简历仍使用一个完整正文编辑框。
